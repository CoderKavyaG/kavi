...
export default function LikeButton({ slug }) {
  ...
  if (loading) {
    return (
      <button disabled className="px-3 py-2 rounded bg-[var(--surface-color)] border border-[var(--border-color)] opacity-60">
        Like • …
      </button>
    )
  }
  ...
  return (
    <button
      onClick={onLike}
      disabled={liked || busy}
      className={`inline-flex items-center gap-2 px-3 py-2 rounded border transition
        ${liked ? 'bg-[var(--accent-bg)] border-[var(--accent-color)] text-[var(--accent-color)]'
                : 'bg-[var(--surface-color)] border-[var(--border-color)] hover:opacity-80'}`}
      title={liked ? 'You liked this' : 'Like this post'}
    >
      <span>❤️</span>
      <span>{count}</span>
